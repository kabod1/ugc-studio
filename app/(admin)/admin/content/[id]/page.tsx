...
export default function AdminContentDetailPage() {
  ...
  async function fetchData() {
    try {
      const res = await fetch(`/api/admin/data?table=content_submissions&id=${id}&select=*,creator_profiles(display_name),campaigns(title,brands(company_name)),content_comments(*,profiles(full_name))`)
  ...
  async function handleModerate(action: "approve" | "reject") {
    const res = await fetch("/api/admin/moderate", {
      ...
      body: JSON.stringify({ type: "content", id, action }),
    })
    if (res.ok) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => handleModerate("approve")} className="px-4 py-2 rounded-md bg-green-600 text-white text-sm hover:bg-green-700">Approve</button>
        <button onClick={() => handleModerate("reject")} className="px-4 py-2 rounded-md border border-destructive text-destructive text-sm hover:bg-destructive/10">Reject</button>
      </div>
