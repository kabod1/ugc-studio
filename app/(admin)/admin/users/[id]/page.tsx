  ...
  async function handleAction(action: "verify" | "ban") {
    ...
    try {
      const res = await fetch("/api/admin/moderate", {
        ...
        body: JSON.stringify({ type: "user", id, action }),
      })
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-card border rounded-lg p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><p className="text-sm text-muted-foreground">Active</p><p className="font-medium">{user.is_active !== false ? "Yes" : "Banned"}</p></div>
          <div><p className="text-sm text-muted-foreground">Phone</p><p className="font-medium">{user.phone || "Not set"}</p></div>
          <div><p className="text-sm text-muted-foreground">Timezone</p><p className="font-medium">{user.timezone || "Not set"}</p></div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={() => handleAction("ban")} disabled={acting}
          ...
          <Ban className="h-4 w-4" /> {user.is_active !== false ? "Ban User" : "Unban User"}
        </button>
      </div>
    </div>
  )
